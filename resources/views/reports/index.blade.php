    ...
    <form method="GET" action="{{ route('reports.index') }}" class="mb-4">
        <div class="form-group">
            ...
            <select name="date_filter" id="date_filter" class="form-control" onchange="toggleCustomDateFields(this.value)">
                ...
                <option value="last_3_months" {{ $dateFilter == 'last_3_months' ? 'selected' : '' }}>Последние 3 месяца</option>
        ...
        <div id="custom-date-fields" style="display: {{ $dateFilter == 'custom' ? 'block' : 'none' }};">
            <div class="form-group">
                ...
                <input type="date" name="start_date" id="start_date" class="form-control" value="{{ isset($startDate) ? $startDate->format('Y-m-d') : '' }}">
            </div>
            <div class="form-group">
                <label for="end_date">Дата окончания:</label>
                <input type="date" name="end_date" id="end_date" class="form-control" value="{{ isset($endDate) ? $endDate->format('Y-m-d') : '' }}">
            </div>
        </div>
